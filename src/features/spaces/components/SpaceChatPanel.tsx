...
import { Send, Sparkles, Loader2, ArrowUpRight, Map, Share2, Layers, ListChecks, Briefcase, Upload, Paperclip, X, Mic, MicOff } from 'lucide-react';
...
const ARTIFACT_ICON = { roadmap: Map, mindmap: Share2, flashcards: Layers, document: Share2, todo: ListChecks, career: Briefcase } as const;
...
function ArtifactOpenCard({ message, onOpen }: { message: SpaceMessage; onOpen: (view: ActiveView) => void }) {
  ...
  // Career Intelligence is the one artifact type that isn't a split-panel in-space view — it's
  // its own dedicated page (multiple roles get toggled there, not embedded per-space), so
  // "Open" navigates there instead of flipping the space's local activeView state.
  function handleOpen() {
    if (artifactType === 'career') {
      navigate(`/app/career?role=${artifactId}`);
    } else {
      onOpen({ type: artifactType, id: artifactId } as ActiveView);
    }
  }
  return (
    <button
...
export function SpaceChatPanel({
  ...
  onUploadJobDescription: (file: File) => void;
  onSkipMaterial: () => void;
  isFlowBusy: boolean;
  ...
  // The career flow's merged role/job-description question (step 0, after the resume) accepts a
  // JD PDF as an alternative to typing the role — unlike the resume step, it stays in normal
  // text-input mode so a paperclip button sits next to the composer instead of replacing it.
  const isCareerJdStep = flow?.kind === 'career' && flow.step === 0 && !flow.awaitingMaterial;
        ...
        {flow?.awaitingMaterial ? (
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {flow?.materialRequired ? 'Upload Resume' : 'Upload PDF'}
            </button>
            {!flow?.materialRequired && (
              <button
          ...
          <div className="flex items-end gap-2">
            ...
            {isCareerJdStep && (
              <>
                <Tooltip content="Attach the job description as a PDF instead">
                  <button
                    onClick={() => jdInputRef.current?.click()}
                    disabled={isBusy}
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-ink-200 bg-white text-ink-500 transition-colors hover:bg-ink-50 disabled:cursor-not-allowed disabled:opacity-40"
                    aria-label="Attach job description PDF"
                  >
                ...
                <input
                  ...
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    e.target.value = '';
                    if (file) onUploadJobDescription(file);
                  }}
                />
              </>
            )}
